class Path {
	...
	Path addPoint(int x, int y) {
		int[][] newPath = new int[path.length + 1][2];//creates a new path array that's bigger by 1
		System.arraycopy(path, 0, newPath, 0, path.length);  //copies everything in current path from index 0 to the larger newPath starting at index 0
	...
	void addPath(Path p) {
		for(int i = 0; i < p.numOfPoints(); i++) {
			this.addPoint(p.path[i][0], p.path[i][1]); //keeps adding points to the current path since addPoint() returns the new path each time 
		}
	}
	...
	double getLength() {
		...
		for(int i = 0; i < path.length-1; i++) {
			//distance = Math.sqrt((path[i+1][0]-path[i][0])*(path[i+1][0]-path[i][0]) + (path[i+1][1]-path[i][1])*(path[i+1][1]-path[i][1]));
			distance = Math.sqrt(Math.pow(path[i+1][0]-path[i][0], 2) + Math.pow(path[i+1][1]-path[i][1], 2));
		}
				sum = sum + distance; 
		return sum;
	}
